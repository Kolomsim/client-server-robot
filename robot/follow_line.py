...
class LineDetect:
    def __init__(self):
        ...
        text_path = '/home/'+self.user_name+'/transbot_ws/src/transbot_linefollow/scripts'
        ...
        self.sub_scan = rospy.Subscriber('/scan', LaserScan, self.registerScan, queue_size=10)
        ...
        self.linear = 0.3
        self.PID_init()
        if self.CameraDevice == "Astra": img_topic = "/camera/rgb/image_raw/compressed"
        else: img_topic = "/usb_cam/image_raw/compressed"
    ...
    def compressed_callback(self, msg):
        ...
        # cv.putText(rgb_img, thread_text, (30, 70), cv.FONT_HERSHEY_SIMPLEX, 0.6, (100, 200, 200), 1)
    ...
    def process(self, rgb_img, action):
        ...
        if self.PWM_init == False: self.ros_ctrl.PWM_Reset()
    ...
    def execute(self, point_x, color_radius):
        self.now_time = time.time()
        if self.now_time - self.laser_time > 2:
            rospy.logwarn("no laser data !!!")
    ...
    def registerScan(self, scan_data):
        ...
        if self.VideoSwitch == False: self.scan_data_calc(scan_data)
        else:
            if abs(self.prev_time - self.laser_time) > 0.5:
                self.scan_data_calc(scan_data)
                self.prev_time = self.laser_time

    def scan_data_calc(self, scan_data):
        # 记录激光扫描并发布最近物体的位置（或指向某点）
        # Record laser scans and post the position of the nearest object (or point to a point)
        ranges = np.array(scan_data.ranges)
        # 按距离排序以检查从较近的点到较远的点是否是真实的东西
        # Sort by distance to check if something real is from a nearer point to a farther point
        self.warning = 1
        # if we already have a last scan to compare to:
        # print(len(np.array(scan_data.ranges)))
        for i in np.argsort(ranges):
            if len(np.array(scan_data.ranges)) == 720:
                # 通过清除不需要的扇区的数据来保留有效的数据
                # Preserve valid data by clearing data from unwanted sectors
